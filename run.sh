...
#
#   This runner only RUNS: parse, manifest, invoke k6, verdict, report. Pool seeding is
#   the sibling command ./prep.sh (data preparation — demand math, seed producers,
#   harvest, activation); the two are separate on purpose: seeding burns shared-env
#   rate-limit budget and must be its own explicit decision.
#
#   scenario   entry name under src/scenarios|mixed/ (.js suffix optional)
...
# READABLE alternative for contract work — structured JSON per request (bodies parsed,
# binary parts replaced), decoded to <run dir>/http-capture.jsonl; same smoke-only and
# delete-after-use discipline:
#   HTTP_CAPTURE=1 ./run.sh trades-create local smoke
set -euo pipefail
...
usage() {
  ...
  echo "seeding:   ./prep.sh <scenario|seed-producer> [env] [profile|ITERATIONS=n]" >&2
  exit 2
}
...
SCENARIO="${1%.js}"; shift
# Data preparation is not running — redirect both old entry styles to ./prep.sh
[[ "$SCENARIO" == "prep" ]] && { echo "ERROR: pool seeding moved to its own command: ./prep.sh $*" >&2; exit 2; }
...
# Entry lookup: measurement entries only (single-API, then mixed). Seed producers are data
# preparation and refuse to run here — without prep's harvest step a producer run would
# silently activate nothing, which is a trap, not a feature.
SCENARIO_FILE=""
for dir in src/scenarios src/mixed; do
  if [[ -f "$dir/${SCENARIO}.js" ]]; then SCENARIO_FILE="$dir/${SCENARIO}.js"; break; fi
done
if [[ -z "$SCENARIO_FILE" && -f "src/seed/${SCENARIO}.js" ]]; then
  echo "ERROR: ${SCENARIO} is a seed producer — seeding is ./prep.sh's job:" >&2
  echo "  ./prep.sh ${SCENARIO} ${ENV_NAME} ITERATIONS=<n>" >&2
  exit 2
fi
ENV_FILE="config/environments/${ENV_NAME}.json"
PROFILE_FILE="profiles/${PROFILE}.json"
[[ -n "$SCENARIO_FILE" ]] || { echo "ERROR: scenario not found: ${SCENARIO} (looked in src/scenarios/ and src/mixed/)" >&2; usage; }
...
# ── Post-run hook: HTTP capture decode is part of this run's reporting; seeding/harvest
#    belongs to ./prep.sh and has no hook here ──
CAPTURE_ON="${HTTP_CAPTURE:-}"
[[ " ${RAW_OVERRIDES[*]:-} " == *" HTTP_CAPTURE="* ]] && CAPTURE_ON=1
[[ -n "$CAPTURE_ON" ]] && scripts/http-capture.sh "$RUN_DIR"
