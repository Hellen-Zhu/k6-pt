...
分界线:**量级是参数,形状按轮次类型分治**——判定轮的形状是"度量衡",测量轮的形状是
"探针"。由此三层:

### 3.1 判定 profile(smoke / baseline / mix / mix-ref / load / soak)—— 形状锁死

产 PASS/FAIL、喂基线的轮次,形状稳定性 = 结论可信度(基线键就是
scenario_env_profile)。portal 只暴露既有标量白名单 `RATE / DURATION / VUS / MAX_VUS`
(run.sh 语义:只覆盖同名标量键,manifest 全记录)。改形状 = 改度量衡,走 PR。

### 3.2 测量 profile(mix-ladder / stress / spike)—— 结构化参数

apiSla:false、不进基线、天生迭代探索。给结构化参数而非自由编辑:
