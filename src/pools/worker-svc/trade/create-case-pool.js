...
// ── Convention-based dat preloading: productId → data/datfiles/products/<productId>/<productId>.dat ──
// Convention over configuration: rows carry only productId and the dat is located by the same-name
...
for (let i = 0; i < createCases.length; i++) {
  const pt = createCases[i].productId || '';
  if (!pt || datBinaries[pt] !== undefined) continue;
  if (!PRODUCT_TYPE_RE.test(pt)) {
    throw new Error(
      `${DATA_FILE} row ${createCases[i].__row}: productId='${pt}' contains illegal characters (only letters/digits/_/- allowed)`
    );
  }
  datBinaries[pt] = open(import.meta.resolve(`${DAT_ROOT}${pt}/${pt}.dat`), 'b');
}

export function getDat(productId) {
  const b = datBinaries[productId];
  if (b === undefined) {
    throw new Error(
      `dat not preloaded: ${productId} — confirm data/datfiles/products/${productId}/${productId}.dat exists` +
      ` and the productId spelling in ${DATA_FILE} matches`
    );
  }
  return b;
}

/** multipart upload filename: by convention, <productId>.dat */
export function datName(productId) {
